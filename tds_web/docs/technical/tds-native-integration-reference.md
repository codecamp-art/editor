...
This document keeps only the TDS details needed for the initial `tds_web` project. For deeper vendor/API details, read the root `tds/` folder and the existing `tds_reporter/` implementation.
...
- `tds_reporter` does not currently expose `currency_code`; `tds_web` must map it.
