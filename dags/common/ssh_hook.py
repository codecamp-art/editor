...
class MSSSHHook(SSHHook):
    def __init__(
        ...
        self.gss_deleg_creds = _coerce_bool(
            gss_deleg_creds
            if gss_deleg_creds is not None
            else extra_options.get("gss_deleg_creds"),
            True,
        )
    ...
    def get_conn(self) -> paramiko.SSHClient:
        ...
        if self.password:
            connect_kwargs["password"] = self.password.strip()
        if self.pkey:
    ...
    *,
    ...
    if exit_status != 0:
        raise AirflowException(
            f"SSH operator error: exit status = {exit_status}, stderr = {agg_stderr.decode('utf-8', errors='replace')}"
        )
