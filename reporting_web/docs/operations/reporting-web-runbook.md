...
- QA: `acl-uireport-qa-ro`
- PROD/DR: `acl-uireport-prod-ro`

PingFederate redirect URLs must match the profile:
...
If PingFederate still returns `there were no client certificates sent in the request`, check the deployment host before changing application code again: the app logs the SSL bundle name, loaded client certificate alias, and KeyManager type used by the token client at startup. The runtime user must be able to read `/app/.cert/server.pem` and `/app/.cert/server.key`, the certificate and private key must match, the certificate chain must include the CA requested by PingFederate, and any load balancer in front of PingFederate must pass client-certificate authentication through correctly. `loaded client certificate alias` only proves the PEM material was loaded. `JSSE selected PingFederate client certificate alias` proves the TLS handshake asked for a client certificate and JSSE selected one. If KeyManager type is logged but neither selected nor fallback log appears during a token request, the token endpoint TLS peer likely did not send a client-certificate `CertificateRequest`. If the logs say JSSE did not select a client certificate alias and fell back to the configured alias, the certificate was loadable but did not match PingFederate's requested CA list.
