...
public final class JsonInPlaceEditor {
    ...
    private static byte[] editInternal(byte[] originalBytes, FileInfo info, List<String> path, String newValue, String expectedOld, Op op) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            // --- NEW: iterate over ALL properties that appear on this logical line ---
            List<PropertyToken> props = extractPropertyTokens(logical);
            if (!props.isEmpty()) {
                // Build the common prefix for current object path once
                List<String> prefix = new ArrayList<>();
                for (PathEntry p : stack) prefix.add(p.key);

                // Maintain dynamic prefix for same-line nested objects/arrays
                List<String> dynPrefix = new ArrayList<>(prefix);

                for (int pIdx = 0; pIdx < props.size(); pIdx++) {
                    PropertyToken pt = props.get(pIdx);

                    List<String> candidate = new ArrayList<>(dynPrefix);
                    candidate.add(pt.key);

                    if (!candidate.equals(path)) {
                        // If this property opens an inline object/array, extend dynPrefix for subsequent siblings
                        String valTrim = pt.valueTrimmed;
                        if (valTrim.startsWith("{") || valTrim.startsWith("[")) {
                            dynPrefix.add(pt.key);
                        }
                        continue;
                    }
                    ...
                    if (expectedOld != null) {
                        ...
                        if (!actual.equals(exp)) {
                            continue; // value mismatch – look for next occurrence
                        }
                    }

                    switch (op) {
                        case REPLACE -> {
                            String repl = pt.prefix + newValue + pt.trailing;
                            props.set(pIdx, pt.withNewToken(repl));
                        }
                        case CLEAR_VALUE -> {
                            String repl = pt.prefix + "\"\"" + pt.trailing;
                            props.set(pIdx, pt.withNewToken(repl));
                        }
                        case DELETE_LINE -> {
                            props.remove(pIdx);
                            // trim possible leading comma on the now-first token
                            if (!props.isEmpty()) {
                                PropertyToken first = props.get(0);
                                String noLeadingComma = first.token.replaceFirst("^\\s*,\\s*", "");
                                props.set(0, first.withNewToken(noLeadingComma));
                            }
                        }
                    }

                    // Reconstruct logical line or remove it entirely if no tokens left
                    if (props.isEmpty()) {
                        // Fall back to original line-removal logic to keep comma housekeeping across lines
                        ...
                        // Join remaining/updated tokens verbatim
                        StringBuilder nb = new StringBuilder();
                        for (PropertyToken t : props) nb.append(t.token);
                        // strip any trailing spaces that might have been left after token removal
                        String rebuilt = nb.toString().replaceFirst("\\s+$", "");
                        lines.set(i, rebuilt + eol);
                    }
                    return assemble(lines, info);
                }
            }

            // Legacy first-property handling remains for simple cases
    ...
    // ---------------------------------------------------------------------------------------------------------
    // Helper structure + extractor for multiple properties on a single logical line
    // ---------------------------------------------------------------------------------------------------------
    private static final class PropertyToken {
        final int start;
        final int end;
        final String key;
        final String token;
        final String prefix;      // everything up to the value (incl colon & spaces)
        final String trailing;    // everything after the value (comma, spaces, comments)
        final String valueTrimmed;
        PropertyToken(int s, int e, String k, String tok, String pref, String trail, String valTrim) {
            start = s; end = e; key = k; token = tok; prefix = pref; trailing = trail; valueTrimmed = valTrim;
        }
        PropertyToken withNewToken(String newTok) { return new PropertyToken(start, start + newTok.length(), key, newTok, prefix, trailing, valueTrimmed); }
    }

    private static List<PropertyToken> extractPropertyTokens(String logical) {
        List<PropertyToken> out = new ArrayList<>();
        int len = logical.length();
        int pos = 0;
        while (pos < len) {
            int tokenStart = pos;
            // advance until we reach a non-whitespace char
            while (pos < len && Character.isWhitespace(logical.charAt(pos))) pos++;
            if (pos < len && logical.charAt(pos) == ',') pos++; // include leading comma if present
            // don't skip the spaces after comma – we want to preserve formatting
            while (pos < len && Character.isWhitespace(logical.charAt(pos))) pos++;
            tokenStart = Math.min(tokenStart, pos);
            if (pos >= len || logical.charAt(pos) != '"') break; // not a property – might be comment or brace

            int keyStart = pos + 1;
            int keyEnd = keyStart;
            while (keyEnd < len) {
                char c = logical.charAt(keyEnd);
                if (c == '\\') {
                    keyEnd += 2; continue;
                }
                if (c == '"') break;
            ...
            if (keyEnd >= len) break;
            String key = logical.substring(keyStart, keyEnd);
            int idx = keyEnd + 1;
            while (idx < len && Character.isWhitespace(logical.charAt(idx))) idx++;
            if (idx < len && logical.charAt(idx) == ':') idx++; else break;
            int afterColonSpacesStart = idx;
            while (idx < len && Character.isWhitespace(logical.charAt(idx))) idx++;
            int valueStart = idx;
            boolean inStr = false, esc = false;
            while (idx < len) {
                char c = logical.charAt(idx);
                if (inStr) {
                    if (esc) { esc = false; }
                    else if (c == '\\') { esc = true; }
                    ...
                    else if (c == '/' && idx + 1 < len && (logical.charAt(idx + 1) == '/' || logical.charAt(idx + 1) == '*')) {
                        // treat comment as part of trailing – stop value here
                        break;
                    }
                }
                idx++;
            ...
            int valueEnd = idx;
            int tokenEnd = valueEnd;
            if (tokenEnd < len && logical.charAt(tokenEnd) == ',') tokenEnd++;
            // consume whitespace after comma
            int tmp = tokenEnd;
            while (tmp < len && Character.isWhitespace(logical.charAt(tmp))) tmp++;
            // include inline comment if present
            if (tmp + 1 < len && logical.charAt(tmp) == '/' && (logical.charAt(tmp + 1) == '/' || logical.charAt(tmp + 1) == '*')) {
                // advance to end of line
                tmp = len;
            }
            tokenEnd = tmp;

            String tokenStr = logical.substring(tokenStart, tokenEnd);
            String prefix = logical.substring(tokenStart, valueStart);
            String valueAndTrailing = logical.substring(valueStart, tokenEnd);
            ValParts vp = splitValueAndRest(valueAndTrailing);
            String valueTrimmed = vp.value.trim();
            String trailing = vp.trailing;
            out.add(new PropertyToken(tokenStart, tokenEnd, key, tokenStr, prefix, trailing, valueTrimmed));
            pos = tokenEnd;
        }
        return out;
    }
